...
public:
  ImuPredictorNode() : Node("imu_predictor_node") {
    ...
    corrected_sub_ = create_subscription<nav_msgs::msg::Odometry>(
        "/localization/registration_state", 10,
        std::bind(&ImuPredictorNode::OnCorrected, this, std::placeholders::_1));

    // 发布 odom 和 TF
    odom_pub_ = create_publisher<nav_msgs::msg::Odometry>("/localization/imu_od", 10);
    tf_broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(*this);

    // 构造函数里加：
    path_pub_ = create_publisher<nav_msgs::msg::Path>("/localization/imu_path", 10);
...
private:
  ...
  void OnCorrected(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    if (last_corrected_valid_) {
      ...
      // dt 合理（0.05s ~ 1.0s，对��� 1~20Hz 的 GICP 更新）
      if (dt > 0.05 && dt < 1.0) {
